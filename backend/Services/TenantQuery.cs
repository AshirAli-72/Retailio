...
{
    /// <summary>
    /// Scopes POS queries to the logged-in admin tenant (user_id).
    /// Each registered admin owns an isolated POS; employees inherit the admin's user_id.
    /// </summary>
    public static class TenantQuery
    {
        ...
        public static IQueryable<customers> ForTenant(this IQueryable<customers> query, int? userId) =>
            HasTenant(userId) ? query.Where(x => x.user_id == userId) : query;
